...
int client_setup(char * committee) {//create and connect a socket to the listening server, char * committee is a string representing server/committee IP
  ...
  int i = connect(sd, results -> ai_addr, results -> ai_addrlen); //connect to socket currently in listening state, client side handshake, binds to address/port, blocks until connection made
